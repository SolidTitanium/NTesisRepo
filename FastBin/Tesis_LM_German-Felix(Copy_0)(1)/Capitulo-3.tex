...
medida $\mu$. Cuando $M$ es una variedad compacta suave, bajo ciertas condiciones sobre
...
\begin{definicion}
    ...
    $\mathbb{R}^{d}$) y la funci\'on $f\cpar{x} = \left\|x\right\|^{2}-\phi^{2}\left(x\right)$ es convexa,
    es decir, para todo $0\leq t \leq 1$ y cualesquiera $x_{1}, x_{2}\in\mathbb{R}^{d}$ se cumple que:
    \begin{equation*}
        f\cpar{t x_{1} + \cpar{1-t}x_{2}} \leq t f\cpar{x_{1}} + \cpar{1-t}f\cpar{x_{2}}.
    \end{equation*}
\end{definicion}
 
Por otra parte decimos que $h$ es semic\'oncava si existe $g: \mathbb{R}^{d} \rightarrow \mathbb{R}$
y $C\geq0$ tal que $g\cpar{x} = h(x) - \frac{C}{2}\cnorm{x}^{2}$ es c\'oncava.
As\'i, tomando $h = \phi$, $g = -f$ y $C = 2$ tenemos que $\phi^{2}$ es semic\'oncava.
Gracias a esto una funci\'on DL $\phi$ tiene un gradiente
...
condici\'on de regularidad sobre $M$\footnote{Por ejemplo, si $M$ es una variedad compacta suave,
...
problema se resuelve al considerar la homolog\'ia del complejo simplicial en cuesti\'on,
...
\begin{equation*}
    \partial_{k}\circ\partial_{k+1}\equiv 0 \text{ para cualquier } k\geq 1.
\end{equation*}
...
Los grupos de homolog\'ia simplicial y los n\'umeros de Betti son propiedades topol\'ogicas invariantes bajo homotop\'ias
(salvo isomorfismos en el caso de los grupos de homolog\'ia simplicial);
es decir, si $K, K'$ son dos complejos simplicales tales que sus realizaciones geom\'etricas son homot\'opicamente
...
Adem\'as de esto, tenemos como consecuencia del Teorema de Reconstrucci\'on (teorema \ref{teoRecon}) el siguiente
...
Afortunadamente se cuenta con una variedad de m\'etodos y resultados que nos ayudan a estimar el soporte de
...
alta probabilidad, de las cubiertas de una muestra en (o bien, cerca) de la variedad en cuesti\'on.
...
$d_{P,m,2}^{2}$ es semic\'oncava, lo cual asegura fuertes propiedades de regularidad en la
